...
### `DefaultEnv <VAR> = <VALUE>`

```qml
//@ pragma DefaultEnv GTK_THEME = Adwaita
//@ pragma DefaultEnv QT_SCALE_FACTOR = 1
```

Sets a default environment variable before the QML engine starts, but only if the variable is not already set in the environment.

Multiple `DefaultEnv` pragmas are allowed.

The variable name and value are trimmed of surrounding whitespace. The separator must be `=`.
